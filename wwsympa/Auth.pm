...
sub remote_app_check_password {
    ...
    my $password                 = shift;
    my $robot                    = Robot::clean_robot(shift);
    Sympa::Log::Syslog::do_log('debug2', '(%s, ..., %s)', $trusted_application_name, $robot);
...
sub get_one_time_ticket {
	...
	unless (
	    ...
	    Sympa::Log::Syslog::do_log('info', 'Unable to find one time ticket %s',
		$ticket_number);
	    return {'result' => 'not_found'};
	}
    }

    Sympa::Log::Syslog::do_log('debug', 'ticket : %s; result : %s', $ticket_number, $result);
    return {
	'result'      => $result,
	'date'        => $ticket->{'date'},
	'email'       => $ticket->{'email'},
