...
<A HREF="[path_cgi]/show_cert" onClick="winhelp=window.open('','wws_help','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,copyhistory=no,width=600,height=320');winlogin.focus()" TARGET="wws_help"><IMG SRC="[icons_url]/locked.gif" align="center" alt="security info" border=0></A>
  ...
  [IF auth_method=md5]
     ...
     [ENDIF]
     <FONT SIZE=-1><B>Abmelden</B></FONT></A>
     </TD>
  [ELSE]
  ...
  [ELSE]
     ...
     [ENDIF]
       ...
       <FONT SIZE=-1><B>Anmeldung</B></FONT></A>
      </TD>
  [ENDIF]
    </TR>
  </TABLE>
...
<TABLE CELLPADDING=0 CELLSPACING=0 WIDTH="100%" BORDER=0>
  ...
  [IF may_create_list]
   [IF action=create_list_request]
    <TD NOWRAP BGCOLOR="[selected_color]" ALIGN="center">
        <FONT SIZE=-1 COLOR=[bg_color] ><B>Liste anlegen</B></FONT>
    </TD>
   [ELSE]
    <TD NOWRAP BGCOLOR="[light_color]"  ALIGN="center">
	 <A HREF="[path_cgi]/create_list_request" ><FONT SIZE=-1><B>Liste anlegen</B></FONT></A>
    </TD>
   [ENDIF]
  [ENDIF]
  ...
  [IF action=pref]
  <TD NOWRAP BGCOLOR="[selected_color]"  ALIGN="center">
      <FONT SIZE=-1 COLOR=[bg_color] ><B>Einstellungen</B></FONT>
  </TD>
  [ELSE]
  <TD NOWRAP BGCOLOR="[light_color]">
      <A HREF="[path_cgi]/pref/[action]/[list]" ><FONT SIZE=-1><B>Einstellungen</B></FONT></A>
  </TD>
  [ENDIF]

  [IF action=which]
  <TD NOWRAP BGCOLOR="[selected_color]" ALIGN="center">
      <FONT SIZE=-1 COLOR=[bg_color] ><B>Ihre Abonnements</B></FONT>
  </TD>
  [ELSE]
  <TD NOWRAP BGCOLOR="[light_color]" ALIGN="center">
      <A HREF="[path_cgi]/which" ><FONT SIZE=-1><B>Ihre Abonnements</B></FONT></A>
   </TD>
   [ENDIF]
  ...
  [IF action=home]
  <TD NOWRAP BGCOLOR="[selected_color]" ALIGN="center"><FONT SIZE=-1 COLOR=[bg_color]><B>Hauptseite</B></FONT></TD>
  [ELSE]
  <TD NOWRAP BGCOLOR="[light_color]" ALIGN="center">
      <A HREF="[path_cgi]/"><FONT SIZE=-1><B>Hauptseite</B></FONT></A>
  </TD>
  [ENDIF]

  [IF action=help]
  <TD NOWRAP BGCOLOR="[selected_color]" ALIGN="center"><FONT SIZE=-1 COLOR=[bg_color]><B>Hilfe</B></FONT></TD>
  [ELSE]
  <TD NOWRAP BGCOLOR="[light_color]" ALIGN="center">
      <A HREF="[path_cgi]/help" ><FONT SIZE=-1><B>Hilfe</B></FONT></A>
  </TD>
  [ENDIF]
</TR>
</TABLE>
