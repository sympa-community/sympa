...
[FOREACH p IN param]
 [IF p->may_edit<>hidden]
  ...
  [IF p->changed=1]
    <TH WIDTH="100%" BGCOLOR="#ff6666">
  [ELSE]
    <TH WIDTH="100%" BGCOLOR="#330099">
  [ENDIF]

   <TABLE WIDTH="100%" BGCOLOR="#3366cc" CELLPADDING="1" CELLSPACING="1"> 
  ...
  [IF is_listmaster]
    ...
    [ENDIF]
  [ENDIF]
  </TH><TH BGCOLOR="#ccccff">
  <A HREF="[path_cgi]/help/editlist#[p->name]" onClick="window.open('','wws_help','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,copyhistory=no,width=400,height=200')" TARGET="wws_help"  ><FONT SIZE="-1">Nápovìda</FONT></A>
  </TH></TR></TABLE>
  ...
  [IF p->default=1]
    <FONT COLOR="#ffffff"><B>výchozí hodnota</B></FONT>
  [ENDIF]
