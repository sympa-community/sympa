...
<A HREF="[path_cgi]/show_cert" onClick="winhelp=window.open('','wws_help','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,copyhistory=no,width=600,height=320');winlogin.focus()" TARGET="wws_help"><IMG SRC="[icons_url]/locked.png" align="center" alt="security info" border=0></A>
  ...
  [IF auth_method=md5]
     ...
     [ENDIF]
     <FONT SIZE=-1><B>Uitloggen</B></FONT></A>
     </TD>
  [ELSE]
...
<TABLE CELLPADDING=0 CELLSPACING=0 WIDTH="100%" BORDER=0>
  ...
  [IF may_create_list]
   [IF action=create_list_request]
    <TD NOWRAP BGCOLOR="[selected_color]" ALIGN="center">
        <FONT SIZE=-1 COLOR=[bg_color] ><B>Maak een nieuwe lijst</B></FONT>
    </TD>
   [ELSE]
    <TD NOWRAP BGCOLOR="[light_color]"  ALIGN="center">
	 <A HREF="[path_cgi]/create_list_request" ><FONT SIZE=-1><B>Maak een nieuwe lijst</B></FONT></A>
    </TD>
   [ENDIF]
  [ENDIF]
  ...
  [IF is_listmaster]
   [IF action=serveradmin]
    <TD NOWRAP BGCOLOR="[selected_color]" ALIGN="center">
        <FONT SIZE=-1 COLOR=[bg_color] ><B>Sympa administratie</B></FONT>
    </TD>
   [ELSE]
    <TD NOWRAP BGCOLOR="[light_color]"  ALIGN="center">
	 <A HREF="[path_cgi]/serveradmin" ><font size=-1><B>Sympa administratie</B></FONT></A>
    </TD>
   [ENDIF]
  [ENDIF]
  ...
  [IF action=pref]
  <TD NOWRAP BGCOLOR="[selected_color]"  ALIGN="center">
      <FONT SIZE=-1 COLOR=[bg_color] ><B>Voorkeuren</B></FONT>
  </TD>
  [ELSE]
  <TD NOWRAP BGCOLOR="[light_color]">
      <A HREF="[path_cgi]/pref/[action]/[list]" ><FONT SIZE=-1><B>Voorkeuren</B></FONT></A>
  </TD>
  [ENDIF]

  [IF action=which]
  <TD NOWRAP BGCOLOR="[selected_color]" ALIGN="center">
      <FONT SIZE=-1 COLOR=[bg_color] ><B>Uw abonnementen</B></FONT>
  </TD>
  [ELSE]
  <TD NOWRAP BGCOLOR="[light_color]" ALIGN="center">
      <A HREF="[path_cgi]/which" ><FONT SIZE=-1><B>Uw abonnementen</B></FONT></A>
   </TD>
   [ENDIF]
