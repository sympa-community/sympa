...
\begin {itemize}
	...
	If you don't know what the hell an authorization scenario is, refer to \ref {scenarios},page~\pageref {scenarios}. Those authorization scenarios are default scenarios but you may look at
        ...
        virtual robot and \dir {/usr/local/sympa-os/expl/\samplelist/scenari} for scenarios
	...
	\item \dir {/usr/local/sympa-os/etc/\samplerobot}\\
        ...
        Those directories for virtual robots have the same structure as  \dir {/usr/local/sympa-os/etc} which is
...
\begin {quote}
    \samplerobot-sympa:   "|/usr/local/sympa-os/bin/sympa.pl sympa@\samplerobot"
    .....
    \samplerobot-listA:   "|/usr/local/sympa-os/bin/sympa.pl listA@\samplerobot"
\end {quote}
...
Scenarios can also be defined for a particular virtual robot (using directory \dir {/usr/local/sympa-os/etc/\texttt{<}robot\texttt{>}/scenari}) or for a list ( \dir {/usr/local/sympa-os/expl/\texttt{<}robot\texttt{>}/\texttt{<}list\texttt{>}/scenari} ).
...
\item Create a \file {/usr/local/sympa-os/etc/your.virtual.domain/robot.conf} configuration file for the virtual robot. Its format is a subset of \file {sympa.conf} and is described in the next section ; a sample \file {robot.conf} is provided.

\item Create a \dir {/usr/local/sympa-os/expl/your.virtual.domain/} directory that will contain the virtual robot mailing lists directories. This directory should have the  \textit {sympa} user as its owner and must have read and write access for this user.
...
loading templates or scenari before searching into \dir {/usr/local/sympa-os/etc} and  \dir {/usr/local/sympa-os/bin/etc}. This allows to define different privileges and a different GUI for a Virtual Robot.
...
robot to a dedicated virtual robot located in the \dir {/usr/local/sympa-os/expl/\samplerobot/} directory. The main benefit of 
...
To create such a virtual robot, you need to create \dir {/usr/local/sympa-os/expl/\samplerobot/} and \file {/usr/local/sympa-os/etc/\samplerobot/} directories ; 
...
(or \file {/usr/local/sympa-os/expl/\samplelist/config} if no virtual robot is defined). 
...
A binary version of the config (\file {/usr/local/sympa-os/expl/\samplerobot/\samplelist/config.bin} is maintained to allow 
