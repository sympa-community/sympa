...
\begin {document}
    ...
    \title {\Huge\bf Sympa \\ \huge\bf Mailing Lists Management Software \\ version 5.2a\\}
    \author {
        Serge Aumont,
        Olivier Sala\"un,
 ...
 \item \textbf {Multilingual} user interface. The full user/admin interface (mail and web)
   ...
   \item \textbf {Template based} user interface. Every web page and service message can
    ...
    \item Privileged operations can be performed by list editors or
        ...
        in the \file {/tmp/sympa/etc/sympa.conf}  global configuration file (listmaster
...
\begin {itemize}
	...
	files as in the distribution.  \dir {/tmp/sympa/bin} may be completely
        overwritten by the \unixcmd {make install} so you must not customize
        templates and authorization scenarios under  \dir {/tmp/sympa/bin}.

	\item \dir {/tmp/sympa/bin/etc}\\
	Here \Sympa stores the default versions of what it will otherwise find
	in \dir {/tmp/sympa/etc} (task models, authorization scenarios, templates and configuration
	files, recognized S/Mime certificates, families).

	\item \dir {/tmp/sympa/etc}\\
	This is your site's configuration directory. Consult
	\dir {/tmp/sympa/bin/etc} when drawing up your own.

	\item \dir {/tmp/sympa/etc/create\_list\_templates/}\\
	List templates (suggested at list creation time).

	\item \dir {/tmp/sympa/etc/scenari/}\\
	...
	If you don't know what the hell an authorization scenario is, refer to \ref {scenarios},page~\pageref {scenarios}. Those authorization scenarios are default scenarios but you may look at
        \dir {/tmp/sympa/etc/\samplerobot/scenari/} for default scenarios of \samplerobot
        virtual robot and \dir {/tmp/sympa/expl/\samplelist/scenari} for scenarios
        specific to a particular list 

	\item \dir {/tmp/sympa/etc/data\_sources/}\\
	...
	\item \dir {/tmp/sympa/etc/list\_task\_models/}\\
	This directory will store your own list task models (see \ref {tasks}, page~\pageref {tasks}).	

	\item \dir {/tmp/sympa/etc/global\_task\_models/}\\
	Contains global task models of yours (see \ref {tasks}, page~\pageref {tasks}).		
	
	\item \dir {/tmp/sympa/etc/web\_tt2/} (used to be \dir {/tmp/sympa/etc/wws\_templates/})\\
	The web interface (\WWSympa) is composed of template HTML
	files parsed by the CGI program. Templates can also 
        be defined for a particular list in \dir {/tmp/sympa/expl/\samplelist/web\_tt2/}
        or in \dir {/tmp/sympa/etc/\samplerobot/web\_tt2/}

	\item \dir {/tmp/sympa/etc/mail\_tt2/} (used to be \dir {/tmp/sympa/etc/templates/})\\
	...
	\item \dir {/tmp/sympa/etc/families/}\\
	Contains family directories of yours (see \ref {ml-creation}, page~\pageref {ml-creation}).
	Families directories can also be created in \dir {/tmp/sympa/etc/\samplerobot/families/}

	\item \dir {/tmp/sympa/etc/\samplerobot}\\
        ...
        in \dir {/tmp/sympa/expl/\samplerobot}).
        Those directories for virtual robots have the same structure as  \dir {/tmp/sympa/etc} which is
        the configuration dir of the default robot. 

	\item \dir {/tmp/sympa/expl}\\
	\Sympa's working directory.

	\item \dir {/tmp/sympa/expl/\samplelist}\\
	...
	page~\pageref {list-directory}). Lists stored in this directory
        ...
        can be stored in \dir {/tmp/sympa/expl/\samplerobot/\samplelist} directory and it
        is managed by \samplerobot virtual robot.

	\item \dir {/tmp/sympa/expl/X509-user-certs}\\
	The directory where Sympa stores all user's certificates

	\item \dir {/tmp/sympa/locale}\\
...
\begin {itemize}
	...
	\item \file {/tmp/sympa/etc/sympa.conf}\\
	The main configuration file.
	See \ref{exp-admin}, page~\pageref{exp-admin}.
	

	\item \file {/tmp/sympa/etc/wwsympa.conf}\\
...
\begin {itemize}
	...
	\item \dir {/tmp/sympa/spool/distribute/bad/}\\
	\Sympa stores rejected messages in this directory
	
	\item \dir {/tmp/sympa/spool/task/}\\
	For storing all created tasks.

	\item \dir {/tmp/sympa/spool/outgoing/}\\
	\file {sympa.pl} dumps messages in this spool to await archiving
	by \file {archived.pl}.

	\item \dir {/tmp/sympa/spool/topic/}\\
...
login directory as \dir {/tmp/sympa}.
...
Sympa comes with default configuration files (templates, scenarios,...) that will be installed in the \dir {/tmp/sympa/bin} directory. 
If you need to customize some of them, you should copy the file first in a safe place, ie in the \dir {/tmp/sympa/etc} directory.
...
\begin {quote}
sympa:             "| /tmp/sympa/bin/queue sympa@\samplerobot"\\
listmaster: 	   "| /tmp/sympa/bin/queue listmaster@\samplerobot"\\
bounce+*:          "| /tmp/sympa/bin/bouncequeue sympa@\samplerobot"\\
...
\begin {quote}
    ...
    \begin {tabular} {ll}
        \mailaddr {\samplelist}:         &
            "|/tmp/sympa/bin/queue \samplelist@\samplerobot"
            \\
        \mailaddr {\samplelist-request}: &
            "|/tmp/sympa/bin/queue \samplelist-request@\samplerobot"
            \\
        \mailaddr {\samplelist-editor}:  &
            "|/tmp/sympa/bin/queue \samplelist-editor@\samplerobot"
            \\
        \mailaddr {\samplelist-owner}:   &
            "|/tmp/sympa/bin/bouncequeue \samplelist@\samplerobot
            \\
        \mailaddr {\samplelist-subscribe}:   &
            "|/tmp/sympa/bin/queue \samplelist-subscribe@\samplerobot@\samplerobot"
            \\
        \mailaddr {\samplelist-unsubscribe}: &
            "|/tmp/sympa/bin/queue \samplelist-unsubscribe@\samplerobot"
            \\
...
as defined in the \file {/tmp/sympa/expl/\samplelist/config}
...
\file {/tmp/sympa/expl/\samplelist/config}.  This address definition
is not compulsory.
...
\end{enumerate}
Example : \file {/tmp/sympa/bin/alias\_manager.pl add \samplelist cru.fr}

\file {/tmp/sympa/bin/alias\_manager.pl} works on the alias file as defined in \file {sympa.conf})
...
\file {/tmp/sympa/bin/alias\_manager.pl} runs a \unixcmd{newaliases} command (via \file {aliaswrapper}), after any changes to aliases file.
...
\begin {quote}
    \samplerobot-sympa:   "|/tmp/sympa/bin/sympa.pl sympa@\samplerobot"
    .....
    \samplerobot-listA:   "|/tmp/sympa/bin/sympa.pl listA@\samplerobot"
\end {quote}
...
The \file {/tmp/sympa/etc/sympa.conf} configuration file contains numerous
...
The \file {/tmp/sympa/etc/sympa.conf} file contains directives in the following
format:
...
\subsection {\cfkeyword {pidfile}} 
	...
	\default {\file {/tmp/sympa/etc/sympa.pid}}
...
\subsection {\cfkeyword {queue}} 
	...
	\example {\dir {/tmp/sympa/spool/msg}}

\subsection {\cfkeyword {queuedistribute}} 
	\index{spool}
	\index{distribution}
	
	\default {\dir {/tmp/sympa/spool/distribute}}
...
\subsection {\cfkeyword {queuemod}}  
	...
	\default {\dir {/tmp/sympa/spool/moderation}}
...
\subsection {\cfkeyword {queueauth}} 
	...
	\default {\dir {/tmp/sympa/spool/auth}}

        This parameter is optional and retained solely for backward compatibility.

\subsection {\cfkeyword {queueoutgoing}} 

	\default {\dir {/tmp/sympa/spool/outgoing}}

	This parameter is optional and retained solely for backward compatibility.

\subsection {\cfkeyword {queuetopic}} 

	\default {\dir {/tmp/sympa/spool/topic}}
...
\subsection {\cfkeyword {queuebounce}} 
	...
	\default {\dir {/tmp/sympa/spool/bounce}}
...
\subsection {\cfkeyword {queuetask}} 
	...
	\default {\dir {/tmp/sympa/spool/task}}
...
\subsection {\cfkeyword {tmpdir}}
        ...
        \default {\dir {/tmp/sympa/spool/tmp}}
...
#define WWSYMPA "/tmp/sympa/bin/wwsympa.fcgi"
...
\begin{verbatim}
     Example :
       	ScriptAlias /sympa /tmp/sympa/bin/wwsympa.fcgi
\end{verbatim}
\end {quote}
...
\begin{verbatim}
     Example :
	FastCgiServer /tmp/sympa/bin/wwsympa.fcgi -processes 2
	<Location /sympa>
...
You may locate this file either in \begin{enumerate}
 	\item \dir {/tmp/sympa/expl/\samplelist/mhonarc-ressources}
	in order to create a specific archive look for a particular list

	\item or \dir {/tmp/sympa/etc/mhonarc-ressources}
...
\dir {/tmp/sympa/expl/\samplelist}). Note that the html archive 
...
\end {quote}
     ...
     to \dir {/tmp/sympa/spool/outgoing/}. (No need to create nonexistent subscribers to receive
...
".rebuild.\samplelist@myhost" in \dir {/tmp/sympa/spool/outgoing}, and make sure that
the owner of this file is \Sympa. 

\begin {quote}
\begin{verbatim}
     example : su sympa -c "touch /tmp/sympa/spool/outgoing/.rebuild.sympa-fr@cru.fr"
\end{verbatim}
\end {quote}
...
\begin {quote}
\begin{verbatim}
	FastCgiServer /tmp/sympa/bin/sympa_soap_server.fcgi -processes 1
	ScriptAlias /sympasoap /tmp/sympa/bin/sympa_soap_server.fcgi
...
The \file {/tmp/sympa/etc/auth.conf} configuration file contains numerous
parameters which are read on start-up of \Sympa. If you change this file, do not forget
that you will need to restart wwsympa.fcgi afterwards. 

The \file {/tmp/sympa/etc/auth.conf} is organised in paragraphs. Each paragraph describes an authentication 
...
The \file {/tmp/sympa/etc/auth.conf} file contains directives in the following format:
...
looks first in the related list directory \dir {/tmp/sympa/expl/\texttt{<}list\texttt{>}/scenari}. If it
does not find the file there, it scans the current robot configuration directory \dir {/tmp/sympa/etc/\samplerobot/scenari}, then the site's configuration directory \dir {/tmp/sympa/etc/scenari},
and finally \dir {/tmp/sympa/bin/etc/scenari}, which is the directory installed by the Makefile.
...
These standard authorization scenarios are located in the \dir {/tmp/sympa/bin/etc/scenari/}
...
\dir {/tmp/sympa/etc/scenari} directory. They will not be overwritten by Sympa release.
Scenarios can also be defined for a particular virtual robot (using directory \dir {/tmp/sympa/etc/\texttt{<}robot\texttt{>}/scenari}) or for a list ( \dir {/tmp/sympa/expl/\texttt{<}robot\texttt{>}/\texttt{<}list\texttt{>}/scenari} ).
...
\subsection {Definition}
	...
	People are selected through an \textindex {LDAP filter} defined in a configuration file. This file must have the extension '.ldap'.It is stored in \dir {/tmp/sympa/etc/search\_filters/}.
...
\textit {Example :} 
\begin {quote}
/tmp/sympa/etc/\samplerobot/scenari/send.intranetorprivate:ignore
\end {quote}
...
\begin {quote}
\begin{verbatim}
sympa@your.virtual.domain:      "| /tmp/sympa/bin/queue sympa@your.virtual.domain"
listmaster@your.virtual.domain: "| /tmp/sympa/bin/queue listmaster@your.virtual.domain"
\end{verbatim}
\end {quote}
...
\begin {quote}
\begin{verbatim}
FastCgiServer /tmp/sympa/bin/wwsympa.fcgi -processes 3 -idle-timeout 120
.....
...
\dir {/tmp/sympa/etc/\samplerobot}. This directory must contain at least a 
\file {robot.conf} file. This files has the same format as  \file {/tmp/sympa/etc/sympa.conf}
...
These settings overwrite the equivalent global parameter defined in \file {/tmp/sympa/etc/sympa.conf}
...
\dir {/tmp/sympa/etc/\samplerobot/web\_tt2/},
\dir {/tmp/sympa/etc/\samplerobot/mail\_tt2/}, 
\dir {/tmp/sympa/etc/\samplerobot/scenari/} directories are searched when
loading templates or scenari before searching into \dir {/tmp/sympa/etc} and  \dir {/tmp/sympa/bin/etc}. This allows to define different privileges and a different GUI for a Virtual Robot.
...
robot to a dedicated virtual robot located in the \dir {/tmp/sympa/expl/\samplerobot/} directory. The main benefit of 
...
To create such a virtual robot, you need to create \dir {/tmp/sympa/expl/\samplerobot/} and \file {/tmp/sympa/etc/\samplerobot/} directories ; 
customize \cfkeyword {host}, \cfkeyword {http\_host} and \cfkeyword {wwsympa\_url} parameters in the \file {/tmp/sympa/etc/\samplerobot/robot.conf} 
...
\begin {enumerate}
	\item \dir {/tmp/sympa/expl/\texttt{<}list\texttt{>}/mail\_tt2/\texttt{<}lang\texttt{>}/\texttt{<}action\texttt{>}.tt2}. 
	\item \dir {/tmp/sympa/expl/\texttt{<}list\texttt{>}/mail\_tt2/\texttt{<}action\texttt{>}.tt2}. 
	\item \dir {/tmp/sympa/etc/\samplerobot/mail\_tt2/\texttt{<}lang\texttt{>}/\texttt{<}action\texttt{>}.tt2}. 
	\item \dir {/tmp/sympa/etc/\samplerobot/mail\_tt2/\texttt{<}action\texttt{>}.tt2}. 
	\item \dir {/tmp/sympa/etc/mail\_tt2/\texttt{<}lang\texttt{>}/\texttt{<}action\texttt{>}.tt2}. 
	\item \dir {/tmp/sympa/etc/mail\_tt2/\texttt{<}action\texttt{>}.tt2}. 
	\item \dir {/tmp/sympa/bin/etc/mail\_tt2/\texttt{<}lang\texttt{>}/\texttt{<}action\texttt{>}.tt2}.
	\item \dir {/tmp/sympa/bin/etc/mail\_tt2/\texttt{<}action\texttt{>}.tt2}.
\end {enumerate}
...
Your list web template files should be placed in the \dir {/tmp/sympa/expl/\samplelist/web\_tt2} 
directory ; your site web templates in \tildedir {/tmp/sympa/etc/web\_tt2} directory.
...
\dir {/tmp/sympa/locale} directory. 
...
file, located in the \dir {/tmp/sympa/etc} directory. The format of this file is 
as follows :
\begin {quote}
\begin{verbatim}
<topic1_name>
...
\Sympa provides several task models stored in \dir {/tmp/sympa/bin/etc/global\_task\_models} 
  and \dir {/tmp/sympa/bin/etc/list\_task\_models} directories.
...
\begin {enumerate}
	\item \dir {/tmp/sympa/expl/\texttt{<}list name\texttt{>}/}
	\item \dir {/tmp/sympa/etc/list\_task\_models/}
	\item \dir {/tmp/sympa/bin/etc/list\_task\_models/}
\end {enumerate}
...
\begin {enumerate}
	\item \dir {/tmp/sympa/etc/global\_task\_models/}
	\item \dir {/tmp/sympa/bin/etc/global\_task\_models/}
\end {enumerate}
...
\file {/tmp/sympa/expl/\samplerobot/\samplelist/config} 
(or \file {/tmp/sympa/expl/\samplelist/config} if no virtual robot is defined). 
...
The \file {/tmp/sympa/expl/\samplelist/subscribers} file is automatically created and
...
\file {/tmp/sympa/expl/\samplelist/info} should contain a detailed text
...
\file {/tmp/sympa/expl/\samplelist/homepage} is the HTML text 
...
\begin {enumerate}
 	\item \dir {/tmp/sympa/expl/\samplelist/data\_sources/\texttt{<}file\texttt{>}.incl}. 
	\item \dir {/tmp/sympa/etc/data\_sources/\texttt{<}file\texttt{>}.incl}. 
	\item \dir {/tmp/sympa/etc/\samplerobot/data\_sources/\texttt{<}file\texttt{>}.incl}.
\end {enumerate} 
...
\begin {enumerate}
 	\item \dir {/tmp/sympa/expl/\samplelist/mail\_tt2/\texttt{<}file\texttt{>}.tt2} 
	\item \dir {/tmp/sympa/etc/mail\_tt2/\texttt{<}file\texttt{>}.tt2}. 
	\item \dir {/tmp/sympa/bin/etc/mail\_tt2/\texttt{<}file\texttt{>}.tt2}.
\end {enumerate}
...
\file {/tmp/sympa/expl/\samplelist/stats} is a text file containing 
...
You may create \file {/tmp/sympa/expl/\samplelist/message.header} and
\file {/tmp/sympa/expl/\samplelist/message.footer} files. Their content
...
The \dir {/tmp/sympa/expl/\samplelist/archives/} directory contains the 
...
List profiles are stored in \dir {/tmp/sympa/etc/create\_list\_templates} or in
\dir {/tmp/sympa/bin/etc/create\_list\_templates} (default of distrib).
...
\dir {/tmp/sympa/etc/create\_list\_templates} and
\dir {/tmp/sympa/bin/etc/create\_list\_templates} directory, then the local profile 
...
the \dir {/tmp/sympa/bin/etc} directory, and you may create your own customized
version in \dir {/tmp/sympa/etc}).
...
For each parameter, you may specify (via the \file {/tmp/sympa/etc/edit\_list.conf}
configuration file) who has the right to edit the parameter concerned ; the default 
\file {/tmp/sympa/bin/etc/edit\_list.conf} is reasonably safe.
...
Domain name of the list, default is the robot domain name set in the related \file {robot.conf} file or in file \file {/tmp/sympa/etc/sympa.conf}.
...
is used otherwise in \dir {/tmp/sympa/etc}.
\end{itemize}
...
Maximum size of a message in 8-bit bytes. The default value is set in
 the \file {/tmp/sympa/etc/sympa.conf} file.
...
\item  \lparam {footer\_type} \texttt {append} 
        ...
        message. \file {/tmp/sympa/expl/\samplelist/message.footer.mime} will be
...
they are stored in \dir {/tmp/sympa/expl/\samplelist/archives/} directory.
...
(\dir {/tmp/sympa/expl/\samplelist/archives/}), used to store these documents.
...
Tagging a message will create a topic information file in the \dir {/tmp/sympa/spool/topic/} spool. Its name is based on the listname and the Message-ID. 
...
A list's shared documents are stored in the \dir {/tmp/sympa/expl/\samplelist/shared}
directory. 
...
\begin {quote}
\begin{verbatim}
	\samplelist-owner: "|/tmp/sympa/bin/bouncequeue \samplelist"
\end{verbatim}
\end {quote}

\file {bouncequeue} (see \ref{binaries}, page~\pageref{binaries}) stores bounces in a
\dir {/tmp/sympa/spool/bounce/} spool.
...
For each mail received, \Sympa extracts its MIME parts in the \dir {/tmp/sympa/spool/tmp/antivirus} directory and
...
\begin{itemize}
  ...
  \item \cfkeyword {cafile} : the path of a bundle of trusted ca certificates. 
        The file \tildefile {/tmp/sympa/bin/etc/ca\-bundle.crt} included in Sympa distribution can be used.
...
the listmaster using \dir {/tmp/sympa/bin/p12topem.pl} script. This script allows
...
\dir {/tmp/sympa/expl/\samplelist/private\_key}. All the list private
...
the \unixcmd {/tmp/sympa/bin/p12topem.pl} script. Use \unixcmd
...
certificates stored in the \dir {/tmp/sympa/expl/X509-user-certs/} directory.
...
Tags the message by creating its topic information file in the \dir {/tmp/sympa/spool/topic/} spool. 
