...
\begin {itemize}
    ...
    \item The possibility of plugging in (commercial) antivirus applications
...
\subsection {\cfkeyword {lang}}   
	...
	\default {us}
        ...
        The is the language for \Sympa. The message
...
You must then set the conf-parameters antivirus_path and antivirus_args
 (see \ref {Antivirus plug-in}, page~\pageref {Antivirus plug-in}.
\Sympa is yet ready to work with Mcafee/uvscan and Fsecure/fsav.
For each mail received, \Sympa splits its parts in a tmp_directory (/'tmpdir'/antivirus)and then calls the antivirus software to check it.
When a virus is detected, \Sympa looks for the virus name in the generated outfile, sends a warning \file 'your_infected_msg.tpl' to the sender of the mail.
The mail is saved as 'bad' and the working directory is deleted except if \Sympa debug option is active.
