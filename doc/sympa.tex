...
\begin {quote}
    ...
    \begin {tabular} {ll}
        ...
        \mailaddr {\samplelist-subscribe}:   &
            "|/usr/local/sympa-os/bin/queue \samplelist-subscribe@\samplerobot"
            \\
...
\subsection {\cfkeyword {automatic\_list\_creation}}  
	...
	If set, Sympa will enable automatic list creation thru family instantiation
	(see \ref {automatic-list-creation}, page~\pageref {automatic-list-creation}.
	Sympa will use the corresponding authorization scenario.
	
	Set to \texttt{none} (default) to disable this feature.

        \example {automatic\_list\_creation listmaster}
...
\subsection {\cfkeyword {pidfile\_creation}} 
	...
	\default {\file {/usr/local/sympa-os/etc/sympa-creation.pid}}
...
\subsection {SQL Named Filters Definition}
       ...
       People are selected through an \textindex {SQL filter} defined in a configuration file. This file must have the extension '.sql'. It is stored in \dir {/usr/local/sympa-os/etc/search\_filters/}.
...
A mail addressed to \textit {auto-cto.50@lists.domain.com} will be queued to the \dir {/usr/local/sympa-os/spool/automatic} spool, 
...
The mail will first be processed by an instance of \file {sympa.pl} dedicated to automatic list creation, then the mail
...
\begin {itemize}
  ...
  \item set the \cfkeyword {automatic\_list\_creation} \file {sympa.conf} parameter to an existing automatic\_list\_creation scenario,

  \item set the \cfkeyword {queueautomatic} \file {sympa.conf} parameter to the spool location where we want these messages to
  be stored (it has to be different from the \dir {/usr/local/sympa-os/spool/msg} spool).

\end {itemize}

You can make Sympa delete automatic lists that were created with zero list members ; to do so
you shoukd set the \cfkeyword {automatic\_list\_empty} parameter to \texttt {delete}.
