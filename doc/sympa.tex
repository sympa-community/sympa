...
\begin {document}
    ...
    \title {\Huge\bf Sympa \\ \huge\bf Mailing Lists Management Software \\ version 5.2a.3\\}
    \author {
        Serge Aumont,
        Olivier Sala\"un,
...
\begin {itemize}
	...
	files as in the distribution.  \dir {/etc/smrsh} may be completely
        overwritten by the \unixcmd {make install} so you must not customize
        templates and authorization scenarios under  \dir {/etc/smrsh}.
	...
	If you don't know what the hell an authorization scenario is, refer to \ref {scenarios},page~\pageref {scenarios}. Those authorization scenarios are default scenarios but you may look at
        ...
        virtual host and \dir {/home/sympa/expl/\samplelist/scenari} for scenarios
        specific to a particular list 

	\item \dir {/home/sympa/etc/data\_sources/}\\
	...
	\item \dir {/home/sympa/etc/web\_tt2/} (used to be \dir {/home/sympa/etc/wws\_templates/})\\
	The web interface (\WWSympa) is composed of template HTML
	files parsed by the CGI program. Templates can also 
        be defined for a particular list in \dir {/home/sympa/expl/\samplelist/web\_tt2/}
        or in \dir {/home/sympa/etc/\samplerobot/web\_tt2/}

	\item \dir {/home/sympa/etc/mail\_tt2/} (used to be \dir {/home/sympa/etc/templates/})\\
	...
	\item \dir {/home/sympa/etc/families/}\\
	Contains family directories of yours (see \ref {ml-creation}, page~\pageref {ml-creation}).
	Families directories can also be created in \dir {/home/sympa/etc/\samplerobot/families/}
	...
	\item \dir {/home/sympa/etc/\samplerobot}\\
        ...
        Those directories for virtual hosts have the same structure as  \dir {/home/sympa/etc} which is
...
\begin {itemize}
	...
	\item \dir {/home/sympa/spool/distribute/}\\
	...
	\item \dir {/home/sympa/spool/distribute/bad/}\\
	\Sympa stores rejected messages in this directory
	...
	\item \dir {/home/sympa/spool/topic/}\\
...
Sympa comes with default configuration files (templates, scenarios,...) that will be installed in the \dir {/etc/smrsh} directory. 
If you need to customize some of them, you should copy the file first in a safe place, ie in the \dir {/home/sympa/etc} directory.
...
\begin {quote}
sympa:             "| /etc/smrsh/queue sympa@\samplerobot"\\
listmaster: 	   "| /etc/smrsh/queue listmaster@\samplerobot"\\
bounce+*:          "| /etc/smrsh/bouncequeue sympa@\samplerobot"\\
...
\begin {quote}
    ...
    \begin {tabular} {ll}
        \mailaddr {\samplelist}:         &
            "|/etc/smrsh/queue \samplelist@\samplerobot"
            \\
        \mailaddr {\samplelist-request}: &
            "|/etc/smrsh/queue \samplelist-request@\samplerobot"
            \\
        \mailaddr {\samplelist-editor}:  &
            "|/etc/smrsh/queue \samplelist-editor@\samplerobot"
            \\
        \mailaddr {\samplelist-owner}:   &
            "|/etc/smrsh/bouncequeue \samplelist@\samplerobot
            \\
        \mailaddr {\samplelist-subscribe}:   &
            "|/etc/smrsh/queue \samplelist-subscribe@\samplerobot@\samplerobot"
            \\
        \mailaddr {\samplelist-unsubscribe}: &
            "|/etc/smrsh/queue \samplelist-unsubscribe@\samplerobot"
            \\
...
\end{enumerate}
Example : \file {/etc/smrsh/alias\_manager.pl add \samplelist cru.fr}

\file {/etc/smrsh/alias\_manager.pl} works on the alias file as defined in \file {sympa.conf})
...
\file {/etc/smrsh/alias\_manager.pl} runs a \unixcmd{newaliases} command (via \file {aliaswrapper}), after any changes to aliases file.
...
\begin {quote}
    \samplerobot-sympa:   "|/etc/smrsh/sympa.pl sympa@\samplerobot"
    .....
    \samplerobot-listA:   "|/etc/smrsh/sympa.pl listA@\samplerobot"
\end {quote}
...
\subsection {\cfkeyword {queue}} 
	...
	\example {\dir {/home/sympa/spool/msg}}

\subsection {\cfkeyword {queuedistribute}} 
	\index{spool}
	\index{distribution}
	
	\default {\dir {/home/sympa/spool/distribute}}
...
\subsection {\cfkeyword {queuetopic}} 
	...
	\default {\dir {/home/sympa/spool/topic}}
...
Scenarios can also be defined for a particular virtual host (using directory \dir {/home/sympa/etc/\texttt{<}robot\texttt{>}/scenari}) or for a list ( \dir {/home/sympa/expl/\texttt{<}robot\texttt{>}/\texttt{<}list\texttt{>}/scenari} ).
...
\begin {quote}
\begin{verbatim}
sympa@your.virtual.domain:      "| /etc/smrsh/queue sympa@your.virtual.domain"
listmaster@your.virtual.domain: "| /etc/smrsh/queue listmaster@your.virtual.domain"
bounce+*@your.virtual.domain:          "| /etc/smrsh/bouncequeue sympa@your.virtual.domain"\\
\end{verbatim}
\end {quote}
...
\item Create a \file {/home/sympa/etc/your.virtual.domain/robot.conf} configuration file for the virtual host. Its format is a subset of \file {sympa.conf} and is described in the next section ; a sample \file {robot.conf} is provided.

\item Create a \dir {/home/sympa/expl/your.virtual.domain/} directory that will contain the virtual host mailing lists directories. This directory should have the  \textit {sympa} user as its owner and must have read and write access for this user.
...
\dir {/home/sympa/etc/\samplerobot/web\_tt2/},
\dir {/home/sympa/etc/\samplerobot/mail\_tt2/}, 
\dir {/home/sympa/etc/\samplerobot/scenari/} directories are searched when
loading templates or scenari before searching into \dir {/home/sympa/etc} and  \dir {/home/sympa/bin/etc}. This allows to define different privileges and a different GUI for a Virtual Host.
...
robot to a dedicated virtual host located in the \dir {/home/sympa/expl/\samplerobot/} directory. The main benefit of 
...
To create such a virtual host, you need to create \dir {/home/sympa/expl/\samplerobot/} and \file {/home/sympa/etc/\samplerobot/} directories ; 
...
\begin {enumerate}
	\item \dir {/home/sympa/expl/\texttt{<}list\texttt{>}/mail\_tt2/\texttt{<}lang\texttt{>}/\texttt{<}action\texttt{>}.tt2}. 
	\item \dir {/home/sympa/expl/\texttt{<}list\texttt{>}/mail\_tt2/\texttt{<}action\texttt{>}.tt2}. 
	\item \dir {/home/sympa/etc/\samplerobot/mail\_tt2/\texttt{<}lang\texttt{>}/\texttt{<}action\texttt{>}.tt2}. 
	\item \dir {/home/sympa/etc/\samplerobot/mail\_tt2/\texttt{<}action\texttt{>}.tt2}. 
	\item \dir {/home/sympa/etc/mail\_tt2/\texttt{<}lang\texttt{>}/\texttt{<}action\texttt{>}.tt2}. 
	\item \dir {/home/sympa/etc/mail\_tt2/\texttt{<}action\texttt{>}.tt2}. 
	\item \dir {/home/sympa/bin/etc/mail\_tt2/\texttt{<}lang\texttt{>}/\texttt{<}action\texttt{>}.tt2}.
	\item \dir {/home/sympa/bin/etc/mail\_tt2/\texttt{<}action\texttt{>}.tt2}.
\end {enumerate}
...
Your list web template files should be placed in the \dir {/home/sympa/expl/\samplelist/web\_tt2} 
directory ; your site web templates in \tildedir {/home/sympa/etc/web\_tt2} directory.
...
\dir {/home/sympa/locale} directory. 
...
(or \file {/home/sympa/expl/\samplelist/config} if no virtual host is defined). 
...
A binary version of the config (\file {/home/sympa/expl/\samplerobot/\samplelist/config.bin} is maintained to allow 
...
\begin {enumerate}
 	\item \dir {/home/sympa/expl/\samplelist/data\_sources/\texttt{<}file\texttt{>}.incl}. 
	\item \dir {/home/sympa/etc/data\_sources/\texttt{<}file\texttt{>}.incl}. 
	\item \dir {/home/sympa/etc/\samplerobot/data\_sources/\texttt{<}file\texttt{>}.incl}.
\end {enumerate} 
...
\begin {enumerate}
 	\item \dir {/home/sympa/expl/\samplelist/mail\_tt2/\texttt{<}file\texttt{>}.tt2} 
	\item \dir {/home/sympa/etc/mail\_tt2/\texttt{<}file\texttt{>}.tt2}. 
	\item \dir {/home/sympa/bin/etc/mail\_tt2/\texttt{<}file\texttt{>}.tt2}.
\end {enumerate}
...
List profiles are stored in \dir {/home/sympa/etc/create\_list\_templates} or in
\dir {/home/sympa/bin/etc/create\_list\_templates} (default of distrib).
...
Tagging a message will create a topic information file in the \dir {/home/sympa/spool/topic/} spool. Its name is based on the listname and the Message-ID. 
...
\begin {quote}
\begin{verbatim}
	\samplelist-owner: "|/etc/smrsh/bouncequeue \samplelist"
\end{verbatim}
\end {quote}
...
Tags the message by creating its topic information file in the \dir {/home/sympa/spool/topic/} spool. 
