...
sub createList {
    ...
    # check authorization
    my $result = Scenario::request_action($robot, 'create_list', 'md5',
	{   'sender'      => $sender,
...
sub add {
	...
	unless ($list->send_file('welcome', $email, {'auto_submitted' => 'auto-generated'})) {
...
sub del {
    ...
    ## quiet del.
    unless ($quiet || $action =~ /quiet/i) {
	unless ($list->send_file('removed', $email, {'auto_submitted' => 'auto-generated'})) {
	    &Log::do_log('notice',"Unable to send template 'removed' to $email");
	}
    }
    
    
    &Log::do_log('info', 'DEL %s %s from %s accepted (%d subscribers)', $listname, $email, $sender, $list->total);
    if ($action =~ /notify/i) {
	unless ($list->send_notify_to_owner('notice',{'who' => $email, 
						      'gecos' => "", 
						      'command' => 'del',
