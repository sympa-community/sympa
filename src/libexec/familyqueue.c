...
char *readconf(char *file)
{
    ...
    if ((f = fopen(file, "r")) != NULL) {
        while (fgets(buf, sizeof buf, f) != NULL) {
            ...
            if (strncmp(buf, "queueautomatic", 14) == 0
                && isspace(buf[14])) {
                /* Strip the ending \n */
...
{
    char *queuedir, *listname, *familyname, *qfile;
    int firstline = 1;

    /* Usage : queue list-name family-name */
    if ((argn != 3)) {
        fprintf(stderr,
                "%s: usage error, two arguments expected : list-name family-name\n",
                argv[0]);
        exit(EX_USAGE);
    }
    if (!*(listname = argv[1]))
        exit(EX_USAGE);
    if (!*(familyname = argv[2]))
        exit(EX_USAGE);
