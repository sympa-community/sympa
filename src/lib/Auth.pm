...
# create new entry in one_time_ticket table using a rand as id so later access
# is authenticated
#
...
sub create_one_time_ticket {
    ...
    unless (
        SDM::do_query(
            "INSERT INTO one_time_ticket_table (ticket_one_time_ticket, robot_one_time_ticket, email_one_time_ticket, date_one_time_ticket, data_one_time_ticket, remote_addr_one_time_ticket, status_one_time_ticket) VALUES (%s, %s, %s, %d, %s, %s, %s)",
            SDM::quote($ticket),
            SDM::quote($robot),
            SDM::quote($email),
            time,
            SDM::quote($data_string),
...
sub get_one_time_ticket {
    ...
    unless (
        SDM::do_query(
            "UPDATE one_time_ticket_table SET status_one_time_ticket = %s WHERE (ticket_one_time_ticket=%s)",
            SDM::quote($addr),
            SDM::quote($ticket_number)
        )
        ) {
        Log::do_log('err', 'Unable to set one time ticket %s status to %s',
            $ticket_number, $addr);
    }
