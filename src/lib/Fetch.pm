...
sub get_https{
	...
	unless ( -r ($trusted_ca_file) ||  (-d $trusted_ca_path )) {
	    Sympa::Log::Syslog::do_log ('err',"error : incorrect access to cafile $trusted_ca_file or capath $trusted_ca_path");
	    return undef;
	}
...
sub get_https2{
	...
	Sympa::Log::Syslog::do_log ('debug','Fetch::get_https2 (%s,%s,%s,%s,%s)',$host,$port,$path,$trusted_ca_file,$trusted_ca_path );
...
#	}
	...
	Sympa::Log::Syslog::do_log ('debug',"requesting  $request");
	#my ($buffer) = $ssl_socket->getlines;
	# print STDERR $buffer;
	#Sympa::Log::Syslog::do_log ('debug',"return");
	#return ;

	Sympa::Log::Syslog::do_log ('debug',"get_https reading answer returns :");
	my @result;
	while (my $line = $ssl_socket->getline) {
	    Sympa::Log::Syslog::do_log ('debug',"$line");
	    push  @result, $line;
	} 
