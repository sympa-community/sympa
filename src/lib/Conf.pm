...
sub load {
    ...
    if (_source_has_not_changed({'config_file' => $config_file})
        ...
        if (my $tmp_conf = _load_binary_cache(
            ...
            printf STDERR
                "Binary config file loading failed while loading source file '%s'\n",
                $config_file;
        }
    } else {
        printf
    ...
    if (my $missing_modules_count =
        ...
        printf STDERR
            "Conf::load(): Warning: %n required modules are missing.\n",
...
sub get_robots_list {
    ...
    unless (opendir DIR, $Conf{'etc'}) {
        printf STDERR
            "Conf::load_robots(): Unable to open directory $Conf{'etc'} for virtual robots config\n";
        return undef;
    }
...
sub conf_2_db {
    ...
    unless (opendir DIR, $Conf{'etc'}) {
        printf STDERR
            "Conf::conf2db(): Unable to open directory $Conf{'etc'} for virtual robots config\n";
        return undef;
    }
    ...
    foreach my $robot (keys %{$Conf{'robots'}}) {
        ...
        if ($dir ne '' && !-d $dir) {
            unless (mkdir($dir, 0775)) {
                Log::do_log('err', 'Unable to create directory %s: %s',
                    $dir, $!);
...
sub load_generic_conf_file {
    ...
    for my $index (0 .. $#paragraphs) {
        ...
        unless ($paragraph[0] =~ /^\s*([\w-]+)(\s+.*)?$/) {
            printf STDERR 'Bad paragraph "%s" in %s, ignored', @paragraph,
                $config_file;
        ...
        unless (defined $structure{$pname}) {
            printf STDERR 'Unknown parameter "%s" in %s, ignored', $pname,
                $config_file;
            return undef if $on_error eq 'abort';
            next;
        }
        ...
        if (defined $admin{$pname}) {
            unless (($structure{$pname}{'occurrence'} eq '0-n')
                ...
                printf STDERR 'Multiple parameter "%s" in %s', $pname,
                    $config_file;
                return undef if $on_error eq 'abort';
            }
        }
...
sub _load_config_file_to_hash {
    ...
    unless (open(IN, $param->{'path_to_config_file'})) {
        printf STDERR
            "Conf::_load_config_file_to_hash(): Unable to open %s: %s\n",
    ...
    while (<IN>) {
        ...
        if (/^(\S+)\s+(.+)$/) {
            ...
            printf STDERR
                "Conf::_load_config_file_to_hash(): Error at line %d: %s\n",
...
sub _remove_unvalid_robot_entry {
    ...
    foreach my $keyword (keys %$config_hash) {
        unless ($valid_robot_key_words{$keyword}) {
            printf STDERR
                "Conf::_remove_unvalid_robot_entry(): removing unknown robot keyword $keyword\n"
                unless ($param->{'quiet'});
    ...
    foreach my $parameter (sort keys %{$param->{'config_hash'}}) {
        ...
        if (defined $old_params{$parameter}) {
            if ($old_params{$parameter}) {
                printf STDERR
                    "Conf::_detect_unknown_parameters_in_config(): Line %d of sympa.conf, parameter %s is no more available, read documentation for new parameter(s) %s\n",
                    $param->{'config_file_line_numbering_reference'}
                    {$parameter}[1], $parameter, $old_params{$parameter};
            } else {
                printf STDERR
                    "Conf::_detect_unknown_parameters_in_config(): Line %d of sympa.conf, parameter %s is now obsolete\n",
                    $param->{'config_file_line_numbering_reference'}
            ...
            printf STDERR
                "Conf::_detect_unknown_parameters_in_config(): Line %d, unknown field: %s in sympa.conf\n",
...
##        next if (defined $params{$parameter}->{'file'} && $params{$parameter}->{'file'} ne $config_file_name);
        unless (defined $param->{'config_hash'}{$parameter}
            ...
            printf STDERR
                "Conf::_detect_missing_mandatory_parameters(): Required field not found in sympa.conf: %s\n",
                $parameter;
            $number_of_errors++;
            next;
        }
...
sub _check_cpan_modules_required_by_config {
    ...
    if ($param->{'config_hash'}{'dkim_feature'} eq 'on') {
        ...
        if ($@) {
            printf STDERR
                "Conf::_check_cpan_modules_required_by_config(): Failed to load Mail::DKIM perl module ; setting 'dkim_feature' to 'off'\n";
...
sub _dump_non_robot_parameters {
    ...
    foreach my $key (keys %{$param->{'config_hash'}}) {
        unless ($valid_robot_key_words{$key}) {
            ...
            printf STDERR
                "Conf::_dump_non_robot_parameters(): Robot %s config: unknown robot parameter: %s\n",
                $param->{'robot'}, $key;
        }
    }
}
...
sub _load_single_robot_config {
    ...
    if (!$force_reload) {
        ...
        unless (-r $config_file) {
            printf STDERR
                "Conf::_load_single_robot_config(): No read access on %s\n",
                $config_file;
            List::send_notify_to_listmaster(
        ...
        unless (
            ...
            printf STDERR
                "Binary config file loading failed. Loading source file '%s'\n",
                $config_file;
            $force_reload = 1;
        }
    }
    if ($force_reload) {
        if (my $config_loading_result = _load_config_file_to_hash(
            ...
            printf STDERR
                "Conf::_load_single_robot_config(): Unable to load %s. Aborting\n",
                $config_file;
            return undef;
        }
...
sub _set_listmasters_entry {
    ...
    if (defined $param->{'config_hash'}{'listmaster'}
        ...
        foreach my $lismaster_address (@emails_provided) {
            if (tools::valid_email($lismaster_address)) {
                ...
                printf STDERR
                    "Conf::_set_listmasters_entry(): Robot %s config: Listmaster address '%s' is not a valid email\n",
        ...
        if ($param->{'main_config'}) {
            printf STDERR
                "Conf::_set_listmasters_entry(): Robot %s config: No listmaster defined. This is the main config. It MUST define at least one listmaster. Stopping here.\n.";
            return undef;
        } else {
    ...
    if ($number_of_email_provided > $number_of_valid_email) {
        printf STDERR
            "Conf::_set_listmasters_entry(): Robot %s config: All the listmasters addresses found were not valid. Out of %s addresses provided, %s only are valid email addresses.\n",
            $param->{'config_hash'}{'host'}, $number_of_email_provided,
            $number_of_valid_email;
        return undef;
    }
...
sub _check_double_url_usage {
    ...
    if (defined $Conf{'robot_by_http_host'}{$host}{$path}) {
        printf STDERR
            "Conf::_infer_robot_parameter_values(): Error: two virtual hosts (%s and %s) are mapped via a single URL '%s%s'\n",
            $Conf{'robot_by_http_host'}{$host}{$path},
            $param->{'config_hash'}{'robot_name'}, $host, $path;
    }
...
sub _save_binary_cache {
    ...
    if ($@) {
        printf STDERR
            'Conf::_save_binary_cache(): Failed to change owner of the binary file %s. error: %s\n',
            $param->{'target_file'}, $@;
        unless ($lock_fh->close()) {
            return undef;
        }
...
sub _save_config_hash_to_binary {
    ...
    unless (
        ...
        printf STDERR
            "Conf::_save_config_hash_to_binary(): Could not save main config %s\n",
