...
sub mail_file {
    ...
    Log::do_log('debug2', 'mail::mail_file(%s, %s, %s)',
    ...
    unless (
        $message_as_string = reformat_message(
...
sub mail_message {
    ...
    Log::do_log(
        'debug',
        'mail::mail_message(from: %s, , file:%s, %s, verp->%s, %d rcpt, last: %s)',
        $from,
        $message->{'filename'},
...
# #      -$from(+) : message from
# #      -$robot(+) : robot
# #      -{verp=>[on|off]} : a hash to introduce verp parameters, starting
# #      just on or off, later will probably introduce optionnal parameters
# #      -@rcpt(+) : recepients
# # OUT : -$numsmtp : number of sendmail process | undef
# #
# ####################################################
...
# 	    undef %rcpt_by_dom ;
# 	    my @tab =  @sendto ; push @sendtobypacket, \@tab ;# do not replace
# 	    this line by push @sendtobypacket, \@sendto !!!
# 	    $numsmtp++ ;
# 	    $nrcpt = $size = 0;
# 	    @sendto = ();
# 	}
#
# 	if ($j && $#sendto >= Conf::get_robot_conf($robot, 'avg') &&
# 	lc("$k[0] $k[1]") ne lc("$l[0] $l[1]")) {
# 	    undef %rcpt_by_dom ;
# 	    $numsmtp++ ;
# 	    my @tab =  @sendto ; push @sendtobypacket, \@tab ;# do not replace
# 	    this line by push @sendtobypacket, \@sendto !!!
...
# 	    @sendto = ();
# 	}
# 	if ($#sendto >= 0 && (($size + length($i)) > $max_arg || $nrcpt >=
# 	Conf::get_robot_conf($robot, 'nrcpt'))) {
# 	    undef %rcpt_by_dom ;
# 	    $numsmtp++  ;
# 	    my @tab =  @sendto ; push @sendtobypacket, \@tab ;# do not replace
# 	    this line by push @sendtobypacket, \@sendto !!!
...
sub mail_forward {
    ...
    unless (
        ...
        Log::do_log('err', 'mail::mail_forward from %s impossible to send',
            $from);
        return undef;
    }
    return 1;
...
sub sendto {
    ...
    Log::do_log(
        'debug',
        'mail::sendto(from : %s,listname: %s, encrypt : %s, verp : %s, priority = %s, last: %s, use_bulk: %s',
        $from,
        $listname,
        $encrypt,
        $verp,
...
sub sending {
    ...
    if ($sign_mode eq 'smime') {
        if ($signed_msg =
            ...
            Log::do_log('notice',
                'mail::sending : unable to sign message from %s', $listname);
            return undef;
        }
    }
    ...
    { # in context wwsympa.fcgi do not send message to reciepients but copy it to standard spool
        ...
        unless (open TMP, ">$sympa_file") {
            Log::do_log('notice', 'mail::sending Cannot create %s : %s',
                $sympa_file, $!);
            return undef;
        }
...
sub smtpto {
    ...
    Log::do_log('debug2',
        'smtpto( from :%s, rcpt:%s, robot:%s,  msgkey:%s, sign_mode: %s  )',
    ...
    if (ref($rcpt) eq 'SCALAR') {
        Log::do_log('debug2', 'mail::smtpto(%s, %s, %s )',
            $from, $$rcpt, $sign_mode);
    } elsif (ref($rcpt) eq 'ARRAY') {
        Log::do_log('debug2', 'mail::smtpto(%s, %s, %s)',
    ...
    while ($opensmtp > Conf::get_robot_conf($robot, 'maxsmtp')) {
        Log::do_log('debug3',
            "mail::smtpto: too many open SMTP ($opensmtp), calling reaper");
...
sub send_in_spool {
    ...
    Log::do_log('debug3', 'mail::send_in_spool(%s,%s, %s)',
        $XSympaFrom, $rcpt);
...
##  passed
##  to mail::reformat_message() separately then attached to reformatted parent
##  message
##  again.  As a result, sub-messages won't be broken.  Since they won't cause
##  mixture
##  of Unicode string (parent message generated by tt2::parse_tt2()) and byte
##  string (sub-messages).
##
##  Note: For compatibility with old style, data passed to
##  mail::reformat_message() already includes
##  sub-message(s).  Then:
##   - When a part has an `X-Sympa-Attach:' header field for internal use, new
##   style,
##     mail::reformat_message() attaches raw sub-message to reformatted parent
##     message again;
...
##  contents of sub-messages are
##  US-ASCII or ISO-8859-1. In other cases customized templates (if any)
##  should be modified so that they
...
sub reformat_message($;$$) {
    ...
    unless (defined $parser) {
        Log::do_log('err',
            "mail::reformat_message: Failed to create MIME parser");
        return undef;
    }
...
sub fix_part {
    ...
    if ($part->head->get('X-Sympa-Attach')) {    # Need re-attaching data.
        ...
        unless (defined $io) {
            Log::do_log('err',
                "mail::reformat_message: Failed to save message : $!");
            return undef;
        }
