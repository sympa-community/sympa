...
sub get_param_value {
    ...
    if (ref($self->{'admin'}{$param}) eq 'ARRAY' and
	...
	$value = _get_single_param_value($pinfo, $self->{'admin'}{$param},$param,$minor_param);
	if ($as_arrayref) {
...
sub distribute_msg {
    ...
    foreach my $field (@{tools::get_list_params($self->{'domain'})->{'rfc2369_header_fields'}->{'format'}}) {
...
sub _append_footer_header_to_part {
    ...
    ## Decode body to Unicode, since HTML::Entities::encode_entities() and newline
...
sub get_nextdigest {
	...
	(Time::Local::timelocal(0, $mm, $hh, $now[3], $now[4], $now[5]) > Time::Local::timelocal(0, $timedigest[1], $timedigest[2], $timedigest[3], $timedigest[4], $timedigest[5]))
        ){
	return 1;
    }
...
sub by_order {
    ($Sympa::ListDef::pinfo{$main::a}{'order'} <=> $Sympa::ListDef::pinfo{$main::b}{'order'}) || ($main::a cmp $main::b);
}
...
sub _save_list_config_file {
	...
	if ((ref ($self->{'admin'}{$key}) eq 'ARRAY') &&
		...
		_save_list_param($self->{'domain'}, $key, $elt, $self->{'admin'}{'defaults'}{$key}, $fd);
	    }
	}else {
	    _save_list_param($self->{'domain'}, $key, $self->{'admin'}{$key}, $self->{'admin'}{'defaults'}{$key}, $fd);
	}
    }
