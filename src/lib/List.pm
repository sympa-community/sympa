...
sub new {
    ...
    my $list = {};
    Log::do_log('debug2', 'List::new(%s, %s, %s)',
    ...
    if ($status == 1
        ...
        unless (defined $list->sync_include_admin()) {
            Log::do_log('err', 'List::new() : sync_include_admin_failed')
                unless ($options->{'just_try'});
        }
...
sub search_list_among_robots {
    ...
    unless ($listname) {
        Log::do_log('err',
            'List::search_list_among_robots() : Missing list parameter');
        return undef;
    }
...
sub extract_verp_rcpt {
    ...
    Log::do_log('debug', 'extract_verp(%s,%s,%s,%s)', $percent, $xseq,
        $refrcpt, $refrcptverp);
...
sub get_owners_email {
    my ($self, $param) = @_;
    Log::do_log(
        'debug3',        'List::get_owners_email(%s,%s)',
        $self->{'name'}, $param->{'ignore_nomail'}
    );
...
sub get_editors_email {
    my ($self, $param) = @_;
    Log::do_log(
        'debug3',        'List::get_editors_email(%s,%s)',
        $self->{'name'}, $param->{'ignore_nomail'}
    );
...
sub update_config_changes {
    ...
    unless (($what eq 'file') || ($what eq 'param')) {
        Log::do_log(
            'err',
            'List::update_config_changes(%s,%s) : %s is wrong : must be "file" or "param".',
            $self->{'name'},
            $what
        );
        return undef;
...
sub _load_config_changes_file {
    my $self = shift;
    Log::do_log('debug3', 'List::_load_config_changes_file(%s)',
        $self->{'name'});
    ...
    while (<FILE>) {
        ...
        if (/^param\s+(.+)\s*$/) {
            ...
            Log::do_log('err',
                'List::_load_config_changes_file(%s) : bad line : %s',
                $self->{'name'}, $_);
            next;
        }
    }
...
sub _save_config_changes_file {
    my $self = shift;
    Log::do_log('debug3', 'List::_save_config_changes_file(%s)',
        $self->{'name'});
...
sub distribute_msg {
    ...
    Log::do_log(
        'debug2',
        'List::distribute_msg(%s, %s, %s, %s, %s, %s, apply_dkim_signature=%s)',
        $self->{'name'},
        $message->{'msg'},
        $message->{'size'},
        $message->{'filename'},
    ...
    for (
        ...
        if ($user_data->{'suspend'} eq '1') {
            if (($user_data->{'startdate'} <= time)
                ...
                ## suspending's data
                restore_suspended_subscription($user->{'email'},
    ...
    foreach my $group (@group_of_msg) {
        ...
        if (@tabrcpt) {
            ...
            unless ($self->send_file('digest', \@tabrcpt, $robot, $param)) {
                Log::do_log('notice',
                    "Unable to send template 'digest' to $self->{'name'} list subscribers"
                );
            }
        }
        ...
        if (@tabrcptplain) {
            ...
            unless (
                $self->send_file(
                    'digest_plain', \@tabrcptplain, $robot, $param
                )
                ) {
                Log::do_log('notice',
                    "Unable to send template 'digest_plain' to $self->{'name'} list subscribers"
                );
            }
        }
        ...
        if (@tabrcptsummary) {
            ...
            {
                Log::do_log('notice',
                    "Unable to send template 'summary' to $self->{'name'} list subscribers"
                );
            }
        }
    }
...
sub send_file {
    ...
    unless (mail::mail_file($filename, $who, $data, $self->{'domain'})) {
        Log::do_log('err',
            "List::send_file, could not send template $filename to $who");
        return undef;
    }
        ...
        for (
            ...
            if (    $user_data
                ...
                if (($user_data->{'startdate'} <= time)
                    ...
                    ## If end date is < time, update the BDD by deleting the
                    ## suspending's data
                    restore_suspended_subscription($user->{'email'}, $name,
                        $robot);
                }
            }
    ...
    foreach my $array_name (keys %$available_rcpt) {
        ...
        if ($#selected_tabrcpt > -1) {
            ...
            unless (defined $result) {
                Log::do_log('err',
                    "List::send_msg, could not send message to distribute from $from (verp desabled)"
                );
                return undef;
            }
        ...
        #  next if  (($array_name eq 'tabrcpt_digest') or ($array_name eq
        #  'tabrcpt_digestlplain') or ($array_name eq 'tabrcpt_summary') or
        #  ($array_name eq 'tabrcpt_nomail')) ;
        next
            if ($array_name =~
...
sub request_auth {
    ...
    @param = @_;
    Log::do_log('debug3',
        'List::request_auth() List : %s,$email: %s cmd : %s',
        $self->{'name'}, $email, $cmd);
    ...
    if (ref($self) eq 'List') {
        ...
        unless ($self->send_file('request_auth', $email, $robot, $data)) {
            Log::do_log('notice',
                "Unable to send template 'request_auth' to $email");
            return undef;
        }
...
sub suspend_subscription {
    ...
    my $data  = shift;
    my $robot = shift;
    Log::do_log('debug2', 'List::suspend_subscription("%s", "%s", "%s" )',
        $email, $list, $data);
...
sub restore_suspended_subscription {
    ...
    my $robot = shift;
    Log::do_log('debug2',
        'List::restore_suspended_subscription("%s", "%s", "%s")',
        $email, $list, $robot);

    unless (
        SDM::do_query(
            "UPDATE subscriber_table SET suspend_subscriber='0', suspend_start_date_subscriber=NULL, suspend_end_date_subscriber=NULL WHERE (user_subscriber=%s AND list_subscriber=%s AND robot_subscriber = %s )",
            SDM::quote($email),
            SDM::quote($list),
            SDM::quote($robot)
        )
        ...
        Log::do_log('err',
            'Unable to restore subscription of user %s to list %s@%s',
            $email, $list, $robot);
        return undef;
    }
...
sub insert_delete_exclusion {
    ...
    my $family = shift;
    Log::do_log('info',
        'List::insert_delete_exclusion("%s", "%s", "%s", "%s", "%s")',
...
sub rename_list_db {
    ...
    Log::do_log('debug', 'List::rename_list_db(%s,%s,%s)',
        $self->{'name'}, $new_listname, $new_robot);
    ...
    Log::do_log('debug', 'List::rename_list_db statement : %s',
...
sub am_i {
    ...
    Log::do_log('debug2', 'List::am_i(%s, %s, %s)',
        $function, $self->{'name'}, $who);
...
sub load_data_sources_list {
    my ($self, $robot) = @_;
    Log::do_log('debug3', 'List::load_data_sources_list(%s,%s)',
        $self->{'name'}, $robot);
...
sub _include_users_remote_sympa_list {
    ...
    Log::do_log(
        'debug',
        'List::_include_users_remote_sympa_list(%s) https://%s:%s/%s using cert %s,',
        $self->{'name'},
        $host,
        $port,
        $path,
            ...
            if (my $plugin = $self->isPlugin($type)) {
                ...
                if ($source->is_allowed_to_sync() || $source_is_new) {
                    my $result =
                        ...
                        $source, $admin->{'default_user_options'});
                    if (defined $result) {
...
sub get_lists {
    ...
    foreach my $robot (@robots) {
        ...
        if (defined $list_cache{'get_lists'}{$robot}) {
            ...
            unless (-d $robot_dir) {
                Log::do_log('err',
                    "unknown robot $robot, Unable to open $robot_dir");
                return undef;
            }
...
sub _load_list_param {
    ...
    Log::do_log('debug3', '_load_list_param(%s,\'%s\',\'%s\')',
        $robot, $key, $value);
...
sub get_cert {
    ...
    if ($format eq 'pem') {
        ...
        Log::do_log('err',
            "List::get_cert(): unknown '$format' certificate format");
        return undef;
    }
...
sub tag_topic {
    ...
    Log::do_log('debug3', 'tag_topic(%s,%s,"%s",%s)', $self->{'name'},
        $msg_id, $topic_list, $method);
...
sub load_msg_topic_file {
    ...
    $msg_id = tools::clean_msg_id($msg_id);
    Log::do_log('debug3', 'List::load_msg_topic_file(%s,%s)',
        $self->{'name'}, $msg_id);
        ...
        if (/^(\S+)\s+(.+)$/io) {
            ...
            if ($keyword eq 'TOPIC') {
                ...
                if ($value =~ /^(editor|sender|auto)$/) {
                    ...
                    Log::do_log(
                        'err',
                        'List::load_msg_topic_file(%s,%s): syntax error in file %s/%s : %s',
                        $queuetopic,
                        $file,
                        $!
                    );
...
sub get_subscription_requests {
    my ($self) = shift;
    Log::do_log('debug2', 'List::get_subscription_requests(%s)',
        $self->{'name'});
...
sub get_subscription_request_count {
    my ($self) = shift;
    Log::do_log('debug2', 'List::get_subscription_requests_count(%s)',
        $self->{'name'});
...
sub search_datasource {
    my ($self, $id) = @_;
    Log::do_log('debug2', 'List::search_datasource(%s,%s)',
        $self->{'name'}, $id);
