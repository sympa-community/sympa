...
#   il faut prÅÈparer une liste de hash avec le nom de liste, le nom de robot, le rÅÈpertoire de la liset pour appeler
...
sub get_which {
	...
	# next unless (($list->{'admin'}{'host'} eq $robot) || ($robot eq '*')) ;
		...
		push @which, $list->{'name'} if ($list->is_user($email));
 	    ...
 	    if ($list->{'admin'}{'user_data_source'} eq 'include2'){
		if (! defined $db_which->{'owner'}{$l}) {
		    push @which, $list->{'name'} if ($list->am_i('owner',$email));
		}elsif ($db_which->{'owner'}{$l} == 1) {
 		    push @which, $l ;
 	    ...
 	    if ($list->{'admin'}{'user_data_source'} eq 'include2'){
		if (! defined $db_which->{'editor'}{$l}) {
		    push @which, $list->{'name'} if ($list->am_i('editor',$email));	       
		}elsif ($db_which->{'editor'}{$l} == 1) {
 		    push @which, $l ;
...
#    next  unless (defined ($p));
	...
	if (($::pinfo{$key}{'occurrence'} =~ /n$/)
	    && $::pinfo{$key}{'split_char'}) {
	    ################" avant de debugger do_edit_list qui crÅÅÈe des nouvelles entrÅÅÈes vides
